...
CREATE TABLE search_source (
 id uuid PRIMARY KEY DEFAULT gen_random_uuid(), tenant_id uuid NOT NULL, name text NOT NULL, source_type text NOT NULL,
...
CREATE TABLE search_query (
 id uuid PRIMARY KEY DEFAULT gen_random_uuid(), tenant_id uuid NOT NULL, session_id uuid NOT NULL, query_text text NOT NULL,
...
CREATE TABLE query_source (
 id uuid PRIMARY KEY DEFAULT gen_random_uuid(), tenant_id uuid NOT NULL, query_id uuid NOT NULL, source_id uuid NOT NULL,
...
CREATE TABLE result_citation (
 id uuid PRIMARY KEY DEFAULT gen_random_uuid(), tenant_id uuid NOT NULL, result_id uuid NOT NULL, locator text NOT NULL,
...
CREATE TABLE ranking_explanation (
 id uuid PRIMARY KEY DEFAULT gen_random_uuid(), tenant_id uuid NOT NULL, result_id uuid NOT NULL, signal text NOT NULL,
 contribution numeric NOT NULL, explanation text NOT NULL,
 FOREIGN KEY (result_id, tenant_id) REFERENCES search_result(id, tenant_id)
);
CREATE TABLE saved_search (
 id uuid PRIMARY KEY DEFAULT gen_random_uuid(), tenant_id uuid NOT NULL, owner_id uuid NOT NULL, name text NOT NULL,
 query_definition jsonb NOT NULL, deleted_at timestamptz, created_at timestamptz NOT NULL DEFAULT now(),
 UNIQUE (id, tenant_id)
);
CREATE TABLE search_feedback (
 id uuid PRIMARY KEY DEFAULT gen_random_uuid(), tenant_id uuid NOT NULL, result_id uuid NOT NULL, user_id uuid NOT NULL,
...
CREATE TABLE search_audit (
 id uuid PRIMARY KEY DEFAULT gen_random_uuid(), tenant_id uuid NOT NULL, query_id uuid NOT NULL, actor_id uuid NOT NULL,
...
CREATE POLICY discovery_session_tenant ON discovery_session USING (tenant_id = (auth.jwt()->>'tenant_id')::uuid) WITH CHECK (tenant_id = (auth.jwt()->>'tenant_id')::uuid);
CREATE POLICY search_source_tenant ON search_source USING (tenant_id = (auth.jwt()->>'tenant_id')::uuid) WITH CHECK (tenant_id = (auth.jwt()->>'tenant_id')::uuid);
CREATE POLICY search_query_tenant ON search_query USING (tenant_id = (auth.jwt()->>'tenant_id')::uuid) WITH CHECK (tenant_id = (auth.jwt()->>'tenant_id')::uuid);
CREATE POLICY query_source_tenant ON query_source USING (tenant_id = (auth.jwt()->>'tenant_id')::uuid) WITH CHECK (tenant_id = (auth.jwt()->>'tenant_id')::uuid);
CREATE POLICY search_result_tenant ON search_result USING (tenant_id = (auth.jwt()->>'tenant_id')::uuid) WITH CHECK (tenant_id = (auth.jwt()->>'tenant_id')::uuid);
CREATE POLICY result_citation_tenant ON result_citation USING (tenant_id = (auth.jwt()->>'tenant_id')::uuid) WITH CHECK (tenant_id = (auth.jwt()->>'tenant_id')::uuid);
CREATE POLICY ranking_explanation_tenant ON ranking_explanation USING (tenant_id = (auth.jwt()->>'tenant_id')::uuid) WITH CHECK (tenant_id = (auth.jwt()->>'tenant_id')::uuid);
CREATE POLICY saved_search_tenant ON saved_search USING (tenant_id = (auth.jwt()->>'tenant_id')::uuid) WITH CHECK (tenant_id = (auth.jwt()->>'tenant_id')::uuid);
CREATE POLICY search_feedback_tenant ON search_feedback USING (tenant_id = (auth.jwt()->>'tenant_id')::uuid) WITH CHECK (tenant_id = (auth.jwt()->>'tenant_id')::uuid);
CREATE POLICY search_audit_tenant ON search_audit USING (tenant_id = (auth.jwt()->>'tenant_id')::uuid) WITH CHECK (tenant_id = (auth.jwt()->>'tenant_id')::uuid);

ALTER TABLE query_source ADD CONSTRAINT uq_query_source_id_tenant UNIQUE(id,tenant_id);
ALTER TABLE result_citation ADD CONSTRAINT uq_result_citation_id_tenant UNIQUE(id,tenant_id);
ALTER TABLE ranking_explanation ADD CONSTRAINT uq_ranking_explanation_id_tenant UNIQUE(id,tenant_id);
ALTER TABLE search_feedback ADD CONSTRAINT uq_search_feedback_id_tenant UNIQUE(id,tenant_id);
ALTER TABLE search_audit ADD CONSTRAINT uq_search_audit_id_tenant UNIQUE(id,tenant_id);